...
//					+ System.getProperty("line.separator")
	...
	public String getElement(int index) throws FeatureExtractorException {
		if ((index >= definition.attributes.length) || (index < 0)) {
			...
			throw new FeatureExtractorException("INTERNAL ERROR: non-existant index for Mean:getElement - claims to have children, but child is null");
		}
	}
		...
		if ((index >= definition.attributes.length) || (index < 0)) {
			...
			try {
				...
				throw new FeatureExtractorException("NumberFormatException " +
						"Width of window must be an integer", e);
			}
		} else if (fe_ != null) {
