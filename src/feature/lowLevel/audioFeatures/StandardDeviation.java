...
public class StandardDeviation extends MetaFeatureFactory {
	...
	public void setWindow(int n) 
			throws FeatureExtractorException {
		if (n <= 1) {
			throw new FeatureExtractorException("Width must be 2 or greater");
		} else {
			sampleWidth = n;
			String tmp;
			if (fe_ != null) {
	...
	public String getElement(int index) 
		...
		if ((index >= definition.getAttributes().length) || (index < 0)) {
			...
			throw new FeatureExtractorException("INTERNAL ERROR: Request for child attribute in Standrad Deviation when the child is null");
		}
	}
	...
	public void setElement(int index, String value) 
		...
		if ((index >= definition.getAttributes().length) || (index < 0)) {
			...
			try {
				...
				if (type <= 1) {
					throw new FeatureExtractorException("Width of the window must be greater than 1");
				} else {
					setWindow(type);
				}
			...
			throw new FeatureExtractorException("Request to set a child in StandardDeviation attrbiute when the child is null");
		}
	}
