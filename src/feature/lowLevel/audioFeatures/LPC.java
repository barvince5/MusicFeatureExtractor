...
public class LPC extends FeatureExtractor {
	...
	public void setElement(int index, String value) throws FeatureExtractorException {
		...
		case 0:
			try {
				...
				throw new FeatureExtractorException("NumberFormatException " +
						"Lambda value must be a double", e);
			} catch (Exception e) {
		...
		case 1:
			try {
				...
				throw new FeatureExtractorException("NumberFormatException " +
						"Number of Dimensions must be an integer", e);
			} catch (Exception e) {
				throw new FeatureExtractorException("Exception "+e.getMessage(), e);
			}
			break;
	 ...
	 */
	public void setNumDimensions(int n) throws FeatureExtractorException {
		if (n < 1) {
			throw new FeatureExtractorException("Must have at least 1 LPC coeffecient - " + n
					+ " provided");
		} else {
			numDimensions = n;
	 ...
	 */
	public void setLambda(double l) throws FeatureExtractorException {
		if (Double.isNaN(l) || Double.isInfinite(l)) {
			throw new FeatureExtractorException("lambda must be a real number");
		} else {
			lambda = l;
		}
	}
