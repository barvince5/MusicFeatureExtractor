...
public class Compactness extends FeatureExtractor {
	...
	public Compactness() {
		...
		super.dependencies[0] = "Magnitude Spectrum";
	...
	public double[] extractFeature(double[] samples, double sampling_rate,
		...
		for (int i = 1; i < mag_spec.length - 1; i++) {
			if ((mag_spec[i - 1] > 0.0) && (mag_spec[i] > 0.0)
					&& (mag_spec[i + 1] > 0.0)) {
				compactness += Math.abs(20.0 * Math.log(mag_spec[i]) 
						- 20.0 * (Math.log(mag_spec[i - 1])	+ Math.log(mag_spec[i]) 
								+ Math.log(mag_spec[i + 1])) / 3.0);
			}
		}
