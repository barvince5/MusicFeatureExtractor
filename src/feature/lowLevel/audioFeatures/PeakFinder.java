...
public class PeakFinder extends FeatureExtractor {
	...
	public String getElement(int index) throws FeatureExtractorException {
		if (index != 0) {
			throw new FeatureExtractorException("INTERNAL ERROR: PeakFinder index != 0 ("
					+ index + ")");
		} else {
	 ...
	 * @throws FeatureExtractorException	thrown if a non-positive threshold is set.
	 */
	public void setPeakThreshold(int peak) throws FeatureExtractorException {
		if (peak <= 0) {
			throw new FeatureExtractorException("PeakFinder peakThreshold must be a positive value.");
		} else {
			peakThreshold = peak;
		}
	}
	...
	public void setElement(int index, String value) throws FeatureExtractorException {
		if (index != 0) {
			...
			try {
				...
				throw new FeatureExtractorException("NuberFormatException " +
						"Peak Threshold Must be an integer", e);
			}
