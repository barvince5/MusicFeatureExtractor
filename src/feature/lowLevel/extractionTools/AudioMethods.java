...
public class AudioMethods
{
		...
		throws AudioMethodsException
	{
		...
		{
			throw new AudioMethodsException("File " + file.getName() + " has an unsupported audio format.");
		}
		catch (IOException ex)
		{
			throw new AudioMethodsException("File " + file.getName() + " is not readable.");
		}
	}
		...
		throws AudioMethodsException
	{
		...
		if ( (bit_depth != 16 && bit_depth != 8 )||
			...
			throw new AudioMethodsException( "Only 8 or 16 bit signed PCM samples with a big-endian\n" +
		...
		if ( ((number_samples == 2 || bytes_per_sample == 2) && (number_of_bytes % 2 != 0)) ||
			...
			throw new AudioMethodsException("Uneven number of bytes for given bit depth and number of channels.");
	...
	public static void writeSamplesToBuffer(double[][] sample_values, int bit_depth, byte[] buffer)
		...
		if (bit_depth != 8 && bit_depth != 16)
			throw new AudioMethodsException( "Bit depth of " + bit_depth + " specified." +
			                     "Only bit depths of 8 or 16 currently accepted." );
		if (buffer == null)
			throw new AudioMethodsException("Null buffer for storing samples provided.");
