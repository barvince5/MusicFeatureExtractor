...
public class FFT
{
	 ...
	 *								a Hanning window.
	 * @throws	FFTException		Throws an exception if the real and imaginary
		...
		throws FFTException
	{
		...
		if (imaginary_input != null)
			if (real_input.length != imaginary_input.length)
				throw new FFTException("Imaginary and real inputs are of different sizes.");

		// Throw an exception if less than three samples are provided
		if (real_input.length < 3)
			throw new FFTException( "Only " + real_input.length + " samples provided.\n" +
