...
import customException.AudioMethodsException;
import customException.AudioSamplesException;
...
public class AudioSamples
{
		...
		throws AudioSamplesException
	{
		...
		if ( audio_file.isDirectory() )
			throw new AudioSamplesException("File " + audio_file.getName() + " is a directory.");
		...
		throws AudioSamplesException
	{
		...
		for (int chan = 0; chan < audio_samples.length; chan++)
			if (audio_samples[chan] == null)
				throw new AudioSamplesException("One or more channels of given audio samples array is empty.");
		int number_samples = audio_samples[0].length;
		for (int chan = 0; chan < audio_samples.length; chan++)
			if (audio_samples[chan].length != number_samples)
				throw new AudioSamplesException( "Different channels of given audio samples array have a\n" +
		                             "different number of samples." );
		if (audio_format == null)
			throw new AudioSamplesException("Null audio format specified for samples.");
		if (audio_format.getChannels() != audio_samples.length)
			throw new AudioSamplesException( "The specified samples have " + audio_samples.length + " channels but\n" +
	 ...
	 *									cannot be extracted from the audio_samples.
	 */
	public AudioSamples(double[][] audio_samples, float sampling_rate, String unique_identifier,
		                 boolean normalize_if_clipped)
		throws AudioSamplesException {
		...
		throws AudioSamplesException
	{
		...
		if (end_sample >= samples.length)
			throw new AudioSamplesException( "Requested audio ending at sample " + end_sample +
	...
	public void saveAudio( File save_file,
		...
		if (save_file == null)
			throw new AudioSamplesException("No file provided to save to.");
	...
	public void setSamples(double[][] new_samples)
		...
		if (new_samples == null)
			throw new AudioSamplesException("An empty set of samples provided.");
		int number_samples = -1;
		for (int chan = 0; chan < new_samples.length; chan++)
		{
			if (new_samples[chan] == null)
				throw new AudioSamplesException("Channel " + chan + " of the given samples is empty.");
			if (number_samples != -1)
				if (number_samples != new_samples[chan].length)
					throw new AudioSamplesException("Different channels have different numbers of samples.");
